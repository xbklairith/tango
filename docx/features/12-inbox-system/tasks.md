...
1. `TestValidateInboxStatusTransition` — verify allowed transitions: `pending->acknowledged`, `pending->resolved`, `acknowledged->resolved`. Verify rejected transitions: `resolved->pending`, `resolved->acknowledged`, `acknowledged->pending`.
2. `TestValidResolutionsForCategory` — verify each category returns the correct set of allowed resolutions per REQ-INB-022.
3. `TestCategoryWakesAgent` — verify `approval_request`, `question`, `decision` return true; `info`, `budget_warning`, `agent_error` return false.
...
- `InboxCategory`, `InboxUrgency`, `InboxStatus`, `InboxResolution` type constants
...
2. The enum types `inbox_category`, `inbox_urgency`, `inbox_status`, `inbox_resolution` exist.
3. The unique partial indexes `uq_inbox_active_budget_per_agent` and `uq_inbox_active_error_per_agent` exist.
...
6. `TestCreateInboxItemOnConflictDoNothing` — insert duplicate budget warning for same agent, verify deduplication.
...
2. `TestInboxServiceResolve_ApprovalRequest` — verify item is resolved, SSE `inbox.item.resolved` event is published, `WakeupService.Enqueue` is called with `inbox_resolved` source and resolution context.
3. `TestInboxServiceResolve_BudgetWarning` — verify item is resolved with `dismissed`, NO wakeup is created.
...
8. `TestInboxServiceCreateBudgetWarning` — verify auto-created item with correct fields and deduplication.
9. `TestInboxServiceCreateAgentError` — verify auto-created item with correct fields and deduplication.
...
- `CreateBudgetWarning(ctx, params) error` — auto-create with ON CONFLICT
- `CreateAgentError(ctx, params) error` — auto-create with ON CONFLICT
...
**Requirements:** REQ-INB-001, REQ-INB-006, REQ-INB-007, REQ-INB-008, REQ-INB-009, REQ-INB-010, REQ-INB-013, REQ-INB-014, REQ-INB-025, REQ-INB-028, REQ-INB-032, REQ-INB-033
**Estimated time:** 60 min
...
- `RegisterRoutes(mux)` — register all 6 routes
...
1. `TestRecordAndEnforce_BudgetWarning80_CreatesInboxItem` — record cost that pushes agent to 80%, verify an inbox item with `category=budget_warning`, `type=budget_threshold_80`, `urgency=high` is created.
2. `TestRecordAndEnforce_BudgetExceeded100_CreatesInboxItem` — record cost that pushes agent to 100%, verify an inbox item with `category=budget_warning`, `type=budget_threshold_100`, `urgency=critical` is created.
...
- In `RecordAndEnforce()`, after threshold detection, call `inboxService.CreateBudgetWarning()`
- The auto-create call should use the transaction-aware queries (`qtx`) so the inbox item is created atomically
...
1. `TestFinalize_RunFailed_CreatesInboxItem` — finalize a run with `status=failed`, verify an inbox item with `category=agent_error`, `type=run_failed`, `urgency=high` is created with `exitCode` and `stderrExcerpt` in payload.
2. `TestFinalize_RunTimedOut_CreatesInboxItem` — finalize with `status=timed_out`, verify inbox item with `type=run_timed_out`.
...
- In `finalize()`, after setting `nextStatus` to `error`, call `inboxService.CreateAgentError()`
